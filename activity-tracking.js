...
function sendEventToDataLayer(event) {
  const config = window.ActivityTracking?.config || {};
  if (typeof config.eventDispatcher === 'function') {
    config.eventDispatcher(event);
  } else {
...
window.ActivityTracking.init = function(userConfig) {
  ...
  if (isNewUser) sendEventToDataLayer({
    event: "first_visit"
  });
  if (isNewVisit) {
    sendEventToDataLayer({
  ...
  if (isNewSource && !isNewVisit) sendEventToDataLayer({
